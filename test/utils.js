...
import { throws, doesNotThrow, ok } from 'assert';
import utils from '../lib/esm/utils';
...
suite('unit testing exported functions of module \'utils.js\'', function () {
  ...
  suite('unit testing function \'escapeRegExpChars\' of module \'utils.js\'', function () {
    test('it should be callable without parameters', function () {
      doesNotThrow(() => { utils.escapeRegExpChars(); });
      ok(utils.escapeRegExpChars()==='');
    });
    test('it should be callable with parameter \'string\' {null}', function () {
      doesNotThrow(() => { utils.escapeRegExpChars(null); });
      ok(utils.escapeRegExpChars(null)==='');
    });
    test('it should be callable with parameter \'string\' {string}', function () {
      const str = 'fun()';
      doesNotThrow(() => { utils.escapeRegExpChars(str); });
      ok(utils.escapeRegExpChars(str)==='fun\\(\\)');
    });
  });
  ...
  suite('unit testing function \'escapeXML\' of module \'utils.js\'', function () {
    test('it should be callable without parameters', function () {
      doesNotThrow(() => { utils.escapeXML(); });
      ok(utils.escapeXML()==='');
    });
    test('it should be callable with parameter \'markup\' {null}', function () {
      doesNotThrow(() => { utils.escapeXML(null); });
      ok(utils.escapeXML(null)==='');
    });
    test('it should be callable with parameter \'markup\' {string}', function () {
      ...
      doesNotThrow(() => { utils.escapeXML(markup); });
      ok(utils.escapeXML(markup)===escaped);
    });
  });
  ...
  suite('unit testing function \'shallowCopy\' of module \'utils.js\'', function () {
    test('it should be callable without parameters', function () {
      doesNotThrow(() => { utils.shallowCopy(); });
      ok(utils.shallowCopy()===undefined);
    });
    test('should be callable with parameters \'to\' {undefined} and \'from\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopy(undefined, null); });
      ok(utils.shallowCopy(undefined, null)===undefined);
    });
    test('should be callable with parameters \'to\' {null} and \'from\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopy(null, null); });
      ok(utils.shallowCopy(null, null)===null);
    });
    test('should be callable with parameters \'to\' {undefined} and \'from\' {...}', function () {
      const to = undefined;
      const from = { 'foo': 'bar', 'baz': [ '1', '2' ], 'bah': { 'hurz': 'gnarf' }};
      doesNotThrow(() => { utils.shallowCopy(to, from); });
      ok(utils.shallowCopy(to, from)===undefined);
    });
    test('should be callable with parameters \'to\' {undefined} and \'from\' {...}', function () {
      const to = null;
      const from = { 'foo': 'bar', 'baz': [ '1', '2' ], 'bah': { 'hurz': 'gnarf' }};
      doesNotThrow(() => { utils.shallowCopy(to, from); });
      ok(utils.shallowCopy(to, from)===null);
    });
    test('should be callable with parameters \'to\' { } and \'from\' {...}', function () {
      const to = {};
      const from = { 'foo': 'bar', 'baz': [ '1', '2' ], 'bah': { 'hurz': 'gnarf' }};
      doesNotThrow(() => { utils.shallowCopy(to, from); });
      ok(JSON.stringify(utils.shallowCopy(to, from))===JSON.stringify(from));
    });
  });
  ...
  suite('unit testing function \'shallowCopyFromList\' of module \'utils.js\'', function () {
    test('it should be callable without parameters', function () {
      doesNotThrow(() => { utils.shallowCopyFromList(); });
      ok(utils.shallowCopyFromList()===undefined);
    });
    test('it should be callable parameters \'to\' {undefined} and \'from\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopyFromList(undefined, null); });
      ok(utils.shallowCopyFromList(undefined, null)===undefined);
    });
    test('it should be callable parameters \'to\' {undefined}, \'from\' {null} and \'list\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopyFromList(undefined, null, null); });
      ok(utils.shallowCopyFromList(undefined, null, null)===undefined);
    });
    test('it should be callable parameters \'to\' {undefined}, \'from\' {null} and \'list\' {Array}', function () {
      const list = [ 'foo', 'bar' ];
      doesNotThrow(() => { utils.shallowCopyFromList(undefined, null, list); });
      ok(utils.shallowCopyFromList(undefined, null, list)===undefined);
    });
    test('it should be callable parameters \'to\' {null} and \'from\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopyFromList(null, null); });
      ok(utils.shallowCopyFromList(null, null)===null);
    });
    test('it should be callable parameters \'to\' {undefined}, \'from\' {null} and \'list\' {null}', function () {
      doesNotThrow(() => { utils.shallowCopyFromList(null, null, null); });
      ok(utils.shallowCopyFromList(null, null, null)===null);
    });
    test('it should be callable parameters \'to\' {null}, \'from\' {null} and \'list\' {Array}', function () {
      const list = [ 'foo', 'bar' ];
      doesNotThrow(() => { utils.shallowCopyFromList(null, null, list); });
      ok(utils.shallowCopyFromList(null, null, list)===null);
    });
    test('it should be callable parameters \'to\' { }, \'from\' {...} and \'list\' {Array}', function () {
      const list = [ 'foo', 'bar' ];
      const to = {};
      const from = { 'foo': 'bar', 'baz': [ '1', '2' ], 'bah': { 'hurz': 'gnarf' }};
      doesNotThrow(() => { utils.shallowCopyFromList(to, from, list); });
      ok(JSON.stringify(utils.shallowCopyFromList(to, from, list))===JSON.stringify({ 'foo': 'bar' }));
    });
  });
  ...
  suite('unit testing object \'cache\' of module \'utils.js\'', function () {
    suite('unit testing function \'set\' of object \'cache\'', function () {
      test('it should be callable without parameters', function () {
        doesNotThrow(() => { utils.cache.set(); });
        doesNotThrow(() => { utils.cache.set(undefined, null); });
        doesNotThrow(() => { utils.cache.set(null, undefined); });
        doesNotThrow(() => { utils.cache.set(null, null); });
      });
      test('it should be callable without parameter \'key\' {string}', function () {
        doesNotThrow(() => { utils.cache.set('key', null); });
        doesNotThrow(() => { utils.cache.set('key', undefined); });
        doesNotThrow(() => { utils.cache.set('key', { 'foo': 'bar' }); });
      });
    });
    suite('unit testing function \'get\' of object \'cache\'', function () {
      test('it should be callable without parameters', function () {
        doesNotThrow(() => { utils.cache.get(); });
        // note this depends on setting key {undefined} to {null} - s.a.
        ok(utils.cache.get()===null);
      });
  ...
  suite('unit testing function \'hyphenToCamel\' of module \'utils.js\'', function () {
    test.skip('it should be callable without parameters', function () {
      ...
      throws(() => { utils.hyphenToCamel(); }, { name: 'TypeError', message });
    });
    test('it should be callable with parameter \'str\' {string}', function () {
      const str = 'some string';
      const strCamel = str;
      doesNotThrow(() => { utils.hyphenToCamel(str); });
      ok(utils.hyphenToCamel(str)===strCamel);

      const other = 'some-string';
      const otherCamel = 'someString';
      doesNotThrow(() => { utils.hyphenToCamel(other); });
      ok(utils.hyphenToCamel(other)===otherCamel);
    });
  });
