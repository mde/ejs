...
controller.BaseController.prototype = new (function () {
    ...
    , _negotiateContent = function (frmt) {
        ...
        if (types.length) {
          ...
          if (wildcard) {
            ...
            // Controllers should have a least one format defined
            if (!contentType) {
  ...
  this.respond = function (content, opts) {
    ...
    if (typeof content != 'string') {
      if (format) {
        ...
        // Otherwise format according to ... format
          ...
          content = response.formatContent(format, content, this);
        }
      }
    ...
    _doResponse.apply(this, [200, {'Content-Type': contentType}, content]);
