    ...
    if(opts.seperator) {
      if(opts.seperator instanceof RegExp) {
        ...
        if(opts.seperator.global) {
          ...
          var ignoreCase = opts.seperator.ignoreCase ? 'i' : ''
            , multiLine = opts.seperator.multiLine ? 'm' : '';

          opts.seperator = new RegExp(opts.seperator.source, 'g' + ignoreCase + multiLine);
        }
        ...
        while((result = opts.seperator.exec(stringToWorkWith)) !== null) {
  ...
  */
  this.camelize = function(string, options) {
    ...
    if(typeof opts === 'boolean') {
      ...
      object.reverseMerge(opts, config);
