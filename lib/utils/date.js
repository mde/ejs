...
var date = new (function () {
  ...
  this.relativeTime = function (dt, options) {
    ...
    switch (true) {
      case diff < 60:
        ret = 'less than a ' + units.minute + ' ago';
        break;
      case diff < 120:
        ret = 'about a ' + units.minute + 'ago';
        break;
      case diff < (45*60):
        ret = parseInt((diff / 60), 10) + ' ' + units.minutes + ' ago';
        break;
      case diff < (2*hour):
        ret = 'about an ' + units.hour + ' ago';
        break;
      case diff < (1*day):
        ret = 'about ' + parseInt((diff / hour), 10) + ' ' + units.hours + ' ago';
        break;
      case diff < (2*day):
        ret = 'one day ago';
        break;
      case diff < (7*day):
        ret = 'about ' + parseInt((diff / day), 10) + ' ' + units.days + ' ago';
        break;
      case diff < (8*day):
        ret = 'one ' + units.week + ' ago';
        break;
      case diff < (1*month):
        ret = 'about ' + parseInt((diff / week), 10) + ' ' + units.weeks + ' ago';
        break;
      default:
        ret = parseInt((diff / day), 10) + ' ' + units.days + ' ago';
        break;
    }
    return ret;
  };
