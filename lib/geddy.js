...
utils.mixin(geddy, new (function () {
  var _started = false
    , _master
    , _worker
    , _startupFunc = function () {
        _worker.startServer(function () {
          if (typeof geddy.afterStart == 'function') {
            geddy.afterStart(_afterStartCallback);
          }
          else {
            _afterStartCallback();
          }
  ...
  this.start = function (options) {
    ...
    _worker.init({clustered: false, logger: c.logger || utils.log}, function () {
      ...
      app.init(_worker.config, function () {
        ...
        if (typeof geddy.beforeStart == 'function') {
          geddy.beforeStart(_startupFunc);
        }
        else {
          _startupFunc();
        }
