...
adapter.Template = function(data) {
  ...
  if(data.data.ext.match(/\.coffee$/)) engine = require('./coffeecup').Template;
  if(data.data.ext.match(/\.hbs$/)) engine = require('./handlebars').Template;
  if(data.data.ext.match(/\.(mustache|ms)$/)) engine = require('./mustache').Template;

  // Create new instance of the rendering engine and return it
  adapter.engine = new engine(data);
