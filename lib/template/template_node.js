...
engine.template = require('./adapters').Template;
engine.textCache = {};

engine.TemplateNode = function(id, data, params, parentNode) {
  this.id = id;
  this.data = data;
...
engine.TemplateNode.prototype = new function() {
  ...
  this.isFinished = function() {
    ...
    if(this.cachedFinish) return true;
    ...
    var childNodes = this.childNodes;
    for (var p in childNodes) {
      if (!childNodes[p].isFinished()) return false;
    }
  ...
  this.loadTemplate = function() {
    ...
    var handleLoaded = function(data) {
      ...
      var templ = new engine.template({ data: self.data, text: data });
    ...
    var cached = engine.textCache[this.data.file];
    ...
    // Get the template from the FS then cache it for subsequent requests
      ...
      fs.readFile(this.data.file, 'utf8', function(err, data) {
        engine.textCache[self.data.file] = data;
        handleLoaded(data);
      });
    }
  };
