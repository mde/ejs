...
getTemplateData = function (templateRoot, partialURL, parentPartial, isLayout) {
  var dirs = []
    , dir
    , key
    , templateData
  ...
  if (parentPartial) {
    dirs.push(parentPartial.dirname);
  }
...
var Partial = function (templatePath, data, parentPartial) {
  var self = this;
  ...
  this.children = [];
  this.adapter = new Adatper();
  this.content = '';

  data.partial = function (templatePath, data) {
    self.addChild(templatePath, data);
    return '###partial###' + self.id;
  };
};
...
Partial.prototype = new (function () {
  ...
  this.render = function (cb) {
      ...
      fs.readFile(, 'utf8', function (err, data) {
        ...
        if (geddy.config.environment !== 'development') {
          cache[self.data.baseNamePath] = content;
        }
        
      });
    // Fetch template content off disk
    // Render self
    // Render children
  };
  ...
  this.addChild = function (templatePath, data) {
    this.children.push(new Partial(templatePath, data, this));
  };
...
var Layout = function (layoutPath, templatePath, data) {
  this.layoutPath = layoutPath;
  Partial.call(this, templatePath, data, null);
...
utils.mixin(Layout.prototype, new (function () {
})());
