...
exports.tags = {
  ...
  */
  ...
  /*
   * selectTagString(optionsArray<Array>, selectedOption, options<Object>)
   *
   * Creates a HTML select tag using the given `optionsArray` to create HTML option elements.
   */
  selectTagString: function (optionsArray, selectedOption, opts) {
    var options = opts || {}
      , optionTags = ''
      , text
      , value
      , textField = options.textField || 'text'
      , valueField = options.textField || 'value'
      , attrs = {};
    ...
    for (var i = 0; i < optionsArray.length; i++) {
      if (typeof optionsArray[i] == 'object') {
        text = optionsArray[i][textField]
        value = optionsArray[i][valueField];
    ...
    return this.contentTagString("select", optionTags, options);
  },
