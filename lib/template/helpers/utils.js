...
exports.registerData = function(data) { Data = geddy.mixin({params: {}}, data); };
...
exports.urls = {
  ...
  genHost: function(options) {
    ...
    if(opts.trailingSlash && !opts.controller && !opts.id && !opts.action) {
      hostname += '/';
    }
  ...
  genPath: function(options) {
    ...
    if(opts.controller) {
      ...
      opts.controller = utils.string.underscorize(opts.controller);

      path += '/' + opts.controller;

      if(!opts.action && !opts.id) {
        opts.action = 'index';
      }
      if(opts.action === 'index') {
        if(opts.trailingSlash) path += '/';
        return path;
      }
    }
    if(opts.id) {
      // If no controller is given attempt to get the current one
      if(!opts.controller && Data.params.controller) {
    ...
    if(opts.action) {
      ...
      if(opts.action === 'show') {
        return opts.trailingSlash ? path += '/' : path;
      } else {
        path += opts.action;
  ...
  genQuery: function(options) {
    ...
    if(!utils.object.isEmpty(opts)) {
      opts = utils.object.toArray(opts);
      ...
      while(++i < opts.length) {
        query += i == 0 ? '?' : '&';
        query += this.escape(opts[i].key) + '=' + this.escape(opts[i].value);
      }
    } else {
      return query;
    }
  ...
  genFragment: function(options) {
    ...
    if(opts.fragment) {
      fragment += '#' + this.escape(opts.fragment);
    }
  ...
  parseOptions: function(options) {
    ...
    // Set up alias's
    opts.domain = opts.domain || opts.host;
    if(opts.host) {
      // Delete it as it's not used
      delete opts.host;
    }
    opts.fragment = opts.fragment || opts.anchor;
    if(opts.anchor) {
      // Delete it as it's not used
      delete opts.anchor;
    }
    ...
    opts = utils.object.merge({ relPath: !opts.domain }, opts);

    // If no `host` is available, and `relPath` is false then we have no idea who
    // the host is
    if(!opts.domain && !opts.relPath) {
    ...
    if(!opts.relPath) {
      protocol += this.genProtocol(opts);
      userinfo += this.genAuth(opts);
      host += this.genHost(opts);
    }
    path += this.genPath(opts);
    fragment += this.genFragment(opts);
    query += this.genQuery(opts);
