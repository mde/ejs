...
_getAssetPath = function (assetType, src, opts) {
  var host
    , basePath
  ...
  if (opts.noPrefix) {
    assetPath = host + path.join('/', basePath, src);
  }
  else {
...
/**
  ...
  @description Creates a HTML select tag using the given `optionsArray` to create HTML option elements.
    Example: selectTag(['open', 'close'], todo.status, { class:'col-md-6', name:'status' })
...
exports.scriptLink = {
  ...
  action: function (source, htmlOptions) {
    var opts = htmlOptions || {}
      , src = _getAssetPath('js', source, opts);

    // remove noPrefix option
    delete opts.noPrefix;
...
exports.styleLink = {
  ...
  action: function (source, htmlOptions) {
    var opts = htmlOptions || {}
      , src = _getAssetPath('css', source, opts);
