...
SnippetTemplate.prototype.getModeMap = function () {
  ...
  if (! SnippetTemplate.modes.CONTROL) {
    SnippetTemplate.modes.CONTROL = 'control';
  }
  if (! Object.keys(m).filter(function(k) { return m[k] === SnippetTemplate.modes.CONTROL; }).length) {
    m['*'] = SnippetTemplate.modes.CONTROL;
  }
  return m;
}
...
SnippetTemplate.prototype.scanTagLine = function (openTag, line, closeTag) {
  ...
  if(mode == EjsTemplate.modes.CONTROL && (name = line.match(/^\s*snippet\s+(\S+)\s*$/))) {
  ...
  if(mode == EjsTemplate.modes.CONTROL && line.match(/^\s*\/snippet\s*$/)) {
