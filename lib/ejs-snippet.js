...
SnippetTemplate.prototype.scanTagLine = function (openTag, line, closeTag) {
  ...
  if(mode == EjsTemplate.modes.EVAL && (name = line.match(/^\*\s*snippet\s+(\S+)\s*$/))) {
  ...
  if(mode == EjsTemplate.modes.EVAL && line.match(/^\*\s*\/snippet\s*$/)) {
...
//    var func = ejs.compile(this.__snippetParsingSource.replace(/<.$/, ''), this.opts);
  ...
  // TODO: maybe if (openTag === ' ') openTag = ''; if (closeTag === ' ') closeTag = '';
