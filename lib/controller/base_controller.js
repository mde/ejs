...
controller.BaseController = function () {
  /**
    ...
    @description The Responder object for this request/response
    cycle.
   */
...
controller.BaseController.prototype = new (function () {
  ...
  */
  ...
  this._handleWithResponder = function (content, opts) {
  ...
  */
  ...
  // Mostly here just for backward-compatibility
  /*
  ...
  */
  this.respond = function (content, opts) {
    ...
    this.responder.formats = this.respondsWith || this.formats;

    // Missing format? Negotiate
    if(!options.format) {
      negotiator = new Negotiator(this.request.headers.accept,
          this.responder.formats, this.params.format);
      negotiated = negotiator.negotiate(options.format);

      utils.mixin(options, negotiated);
    }
    // Missing content type? Look it up
    else if(!options.contentType) {
      options.contentType = mime.lookup(options.format);
    }

    this.responder.outputFormattedContent(content, options);
  };
  ...
  /**
    ...
    @param {Object} [opts] Options.
  */
  this.respondWith = function (content, opts) {
  ...
  */
  this.respondTo = function (strategies) {
    this._handleWithResponder(null, {
