...
controller.BaseController.prototype = new (function () {
    ...
    var self = this
      ...
      , cb = function (formattedContent) {
          self._doResponse(opts.statusCode || 200,
              {'Content-Type': opts.contentType}, formattedContent);
        };

    if (!(opts.format && opts.contentType)) {
      this.negotiator.init(this.request.headers.accept,
          this.respondsWith || this.formats, this.params.format);
      utils.mixin(opts, this.negotiator.negotiate(opts.format) || {});
    }
  ...
  */
  this.respondTo = function (strategies, options) {
    ...
    this.negotiator.init(this.request.headers.accept,
        Object.keys(strategies), this.params.format);
    utils.mixin(opts, this.negotiator.negotiate(opts.format) || {});
