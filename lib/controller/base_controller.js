...
controller.BaseController.prototype = new (function () {
  ...
  */
  ...
  this.flash = function (type, message) {
    if (message) {
      this.session.flash.set(type, message);
    }
    else {
      return this.session.flash.get(type);
    }
  };
  ...
  (function (self) {
    ...
    ['alert', 'error', 'success', 'info'].forEach(function (type) {
      // this.flashAlert -> this.flash('alert' ...
      self['flash' + utils.string.capitalize(type)] = function (message) {
