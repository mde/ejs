...
module.exports = new (function () {
  ...
  this.loadAdapterForModel = function (modelItem, cb) {
    ...
    if (!adapterPath) {
      geddy.log.info('Model adapter not found for ' + name +
          '. Set .adapter for this model, or set model.config.defaultAdapter.');
    }
    else {
      ...
      if (!adapter) {
        config = _getAdapterConfig(this.config.db, adapterName);
  ...
  this.init = function (app, callback) {
    ...
    model.setLocalRequireError('As of Geddy v0.11, DB adapters must be installed locally in your app.\n' +
        'If this is an existing Geddy app, you probably need to run `geddy gen upgrade`.\n' +
        'If this is a new app, set up your DB server and run `geddy jake db:init environment=<environment>`');
    ...
    if (
      !utils.file.existsSync(path.join(cwd, modelDir)) ||
      false === this.config.model
    ) {
      return callback();
    }

    // Set any model properties from the app config
    utils.mixin(model.config, this.config.model);
