        ...
        if (geddy.config.realtime) {
          ...
          , registerEventListener = function (model, event) {
            geddy.model[model].on(event, function (d) {
              ...
              geddy.io.sockets.emit(model+':'+event, (typeof data['toJSON'] === 'function' ) ? data.toJSON() : data);
              geddy.io.sockets.emit(model+':'+event+':'+id, (typeof data['toJSON'] === 'function' ) ? data.toJSON() : data);
            });
          }
