  ...
  this.start = function (callback) {
    ...
    geddy.server.addListener('request', function (req, resp) {
      ...
      dmn.on('error', function (err) {
          ...
          controllerInst = new ErrorController(reqObj, respObj, {}, serverErr);
          controllerInst.respond();
      });
      ...
      try {
        ...
        badRequestError = new errors.BadRequestError(err.message, err.stack);
      }
      ...
      dmn.run(function () {
        ...
        // If we got a bad-reqeust error above, throw it here so it
        // will be handled by our customizable domains-based errors
        if (badRequestError) {
