...
var App = function () {
  ...
  // ==================
    , _runAppLocalInit = function (next) {
        ...
        var doIt = function () {
          ...
          // Found an item to load
          if (fileName) {

            // Bail to next if it's not a usable file
            ...
            if (fileExt == '.coffee' && !usingCoffee) {
              usingCoffee = utils.file.requireLocal('coffee-script');
            }

            // Require the file -- if simple included code, just run it
            ...
            if (typeof initFunc == 'function') {
              ...
              // signature, call it with the doIt as its callback
            ...
            // No exported `init` func, just go next
            else {
              doIt();
            }
          }
          // No more items, done with this init step
          else {
            next();
          }
        };
  ...
  this.handleStaticFile = function (staticPath, reqUrl, reqObj, respObj) {
    var controllerInst;
    ...
    if (fs.statSync(staticPath).isDirectory()) {
      ...
      // Directory with no index file
        ...
        this.handleNotFound(reqUrl, reqObj, respObj);
      }
    }
  ...
  this.handleNotFound = function (reqUrl, reqObj, respObj) {
  ...
  this.handleNoMatchedRoute = function (method, reqUrl, params, reqObj, respObj) {
    ...
    // Nope, static?
      ...
      if (utils.file.existsSync(staticPath)) {
        this.handleStaticFile(staticPath, reqUrl, reqObj, respObj);
      }
      // Nada, 404
      else {
        this.handleNotFound(reqUrl, reqObj, respObj);
