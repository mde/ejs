...
var App = function () {
  ...
  var _getDirList = function (dirname) {
    ...
    , _createActionHelpers = function(item, ctor) {
      // Add controller action helpers
      var ctorActions = new ctor();
      for(var i in ctorActions) {
        actionHelpers.create({controller: utils.string.decapitalize(item.ctorName), action: i});
      }
    }
  ...
  // ==================
    , _registerControllers = function (next) {
        ...
        for (var i = 0; i < dirList.length; i++) {
          ...
          _createActionHelpers(item, ctor);
        }
  ...
  // ================================================
    , _afterStart = function(next) {
        ...
        if (geddy.config.socketIo) {
            geddy.io = require(path.join(cwd, 'node_modules', 'socket.io')).listen(geddy.server);
        }
        try {
