 ...
 * $2: open tag modifier (_=-#)
 */
var _REGEX_STRING_OPEN  = '([^]*?)<%(?!%)([_=#-])?';
/**
 * Find next close tag, skip literals
 * $1: any data before close tag
 * $2: close tag modifier (_-)
 */
var _REGEX_STRING_CLOSE = '^((?:(?![-_%]?%>|<%(?!%))[^]|%%>)*)([-_]?)%>';
// Replace literals, skip extra close tags
var _REGEX_STRING_LITERAL
  = '(?:'
  +   '(?:(?:(<%)%)|(?:%(%>)))'                // $1,$2: match literal
  +   '(?:'
  +     '((?:(?!<%%|[%_-]?%>)[^])*?)'          // $3 match any data to next tag
  +     '(?:(%>)|(?:([-_]%>)(?:\r\n|\r|\n)?))' // $4: plain close <% // $5 special close [-_]%> and slurp \n
  +   ')?'
  + ')'
  + '|(?:(?:[-_]%>(?:\r\n|\r|\n)?)|%>)';       // match any close tag that are not paired with a literal
var _REGEX_STRING_LITERAL_REPL = '$1$2$3$4$5';
...
Template.prototype = {
  createRegex: function () {
    ...
    this.regexTagLiteral = new RegExp(_REGEX_STRING_TAG_LITERAL.replace(/%/g, delim), 'g');
    this.regexTagLiteralReplace = _REGEX_STRING_TAG_LITERAL_REPL;
  },
  ...
  generateSource: function () {
    ...
    var patTagLit = this.regexTagLiteral;
    var replTagLit = this.regexTagLiteralReplace;
    ...
    while (result) {
      ...
      var tag =  res2[1].replace(patTagLit, replTagLit);
      this.scanTagLine(t, tag,res2[2] || ' ');
