...
var fs = require('fs')
  ...
  , _OPTS = [ 'cache', 'filename', 'delimiter', 'scope', 'context'
            , 'debug', 'compileDebug', 'client', '_with', 'views'
            ]
  , _TRAILING_SEMCOL = /;\s*$/
  , _BOM = /^\uFEFF/;
...
exports.resolveInclude = function(name, filename, isdir) {
  var path = require('path')
    ...
    , includePath = isdir ? resolve(filename, name) : resolve(dirname(filename), name)
    , ext = extname(name);
  if (!ext) {
    includePath += '.ejs';
  }
...
function includeFile(path, options) {
  ...
  if(path.substr(0,2) === "~/"){
    if (!opts.views) {
      throw new Error('`include` requires the \'views\' option.');
    }
    opts.filename = exports.resolveInclude(path.substr(2), opts.views, 1);
  }else{
...
function includeSource(path, options) {
  ...
  if(path.substr(0,2) === "~/"){
    ...
    includePath = exports.resolveInclude(path.substr(2), opts.views, 1);
  }else{
...
exports.renderFile = function () {
  ...
  // express auto 'views' option;
  if (data && data.setting ) {
    opts.views = data.setting.views;
  }
  try {
