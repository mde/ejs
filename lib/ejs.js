...
, getLoc: function (index) {
  var substr = this.templateText.substr(0, index);
  var linebreaks = substr.match(/\n/g);
  return {
    line: linebreaks ? linebreaks.length + 1 : 1,
    column: index - substr.lastIndexOf('\n')
  };
}
, compile: function () {
    var src
      ...
      matches.forEach(function (value, index) {
        ...
        if ( line.indexOf('<' + d) === 0        // If it is a tag
          ...
          if (!(closing == d + '>' || closing == '-' + d + '>' || closing == '_' + d + '>')) {
            error = new Error('Could not find matching close tag at: "' + line + '".');
            error.file = self.opts.filename;
            loc = self.getLoc(offset);
            error.line = loc.line;
...
, parseTemplateText: function () {
    ...
    while (result) {
      ...
      if (firstPos !== 0) {
        arr.push([str.substring(0, firstPos), firstPos]);
        str = str.slice(firstPos);
      }

      arr.push([result[0], firstPos]);
