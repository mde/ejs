 ...
 */
var _REGEX_STRING_OPEN  = '^('+_REGEX_ALL_BUT_TAGS+'*)(?:<%(?!%)([__OPENTAG__])?|([_-]?%>))';
/**
...
Template.prototype = {
  ...
  createRegex: function () {
    ...
    this.regexOpen = new RegExp(_REGEX_STRING_OPEN
  ...
  compile: function () {
    ...
    try {
      fn = new Function(self.generateArgumentNames(opts).join(','), src);
    }
    catch(e) {
    ...
    var returnedFn = function (data, callerFnArgs) {
      ...
      var fnArgsList = self.generateArgumentNames(opts).map(function(k) { return fnArgs[k]; });
  ...
  generateArguments: function (data, opts, ejsArgs, callerFnArgs) {
    var r = {
