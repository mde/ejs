...
model.ModelDefinitionBase = function (name) {
  ...
  this.hasMany = function (ctorName) {
    var self = this;
    model[ctorName] = function (query, opts, callback) {
      query = query || {};
      opts = opts || {};
      callback = callback || function(){};
      query[self.name.toLowerCase() + 'id'] = model.id;
      geddy.model[ctorName].all(query, opts, callback);
    }
  };

  this.belongsTo = function (ctorName) {
    model[ctorName] = function (callback) {
      var query = model[ctorName.toLowerCase()+'id'];
      geddy.model[ctorName].load(query, {}, callback);
    }
  }

  this.hasOne = function (ctorName) {i
    var self = this;
    model[ctorName] = function (callback) {
      var query = {};
      query[self.name.toLowerCase() + 'id'] = model.id;
