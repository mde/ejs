...
Worker.prototype = new function () {
  ...
  /* THIS MUST BE DEFINED BY CONSUMERS.
  *  Please overwrite it for your use case.
  *  The property this.router.routes has ROUTE_TIMEOUT
  *  milliseconds to be populated or the worker will fail
  */
  //  This guy actually dispatches the requests with
  //  request and response data to their appropriate handlers*/
  this.router = null;

  // Late binding delegation, router isn't set yet.
  this.map = function () {
    return this.router['map'] && this.router['map'].apply(this.router, arguments);
  };
